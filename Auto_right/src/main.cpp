...
PwmOut PWM_ito(PA_15); //適宜変更 
DigitalOut PHASE_ito(PA_13); //適宜変更
Motor ito(PWM_ito, PHASE_ito, 100, true);
...
sw=!sw; 
 ...
 int start=500, end=5000;//設定
       for(int p = start; p < end; p += 20) {
           pwmri.pulsewidth_us(p);
           pwmle.pulsewidth_us(p);
           wait_ms(40);
       }
